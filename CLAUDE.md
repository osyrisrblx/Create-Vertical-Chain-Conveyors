...
- Target: Create 6.0.x · NeoForge 21.1.219+ · MC 1.21.1 · Java 21
...
Current fast coverage lives under `src/test/java/com/osyris/verticalchainconveyors/`, primarily in `VCCChainConveyorMathTest`, `VCCChainConveyorMovementTest`, `VCCDeferredStatsRefreshTest`, and `VCCVisualSectionsTest`. It checks:
...
- symmetric, dual-axis connection validation for axial and too-steep candidate links
- exhaustive validation reciprocity across every facing pair and a 9×9×9 offset cube
- exact behavior at the 45-degree slope boundary
...
- axis-aware loop interaction bounds for riding, package insertion, and Frogports
- ray hits and misses against every mounted loop interaction plane
- rotation and mirror round trips for every `FACING` value
- indefinite, interval-controlled retry and eventual resolution of delayed target stats
...
- **`ChainConveyorBlockTransformMixin`** — rotates and mirrors the injected `FACING` property for structures, schematics, and contraption transforms.
...
- **`ChainConveyorShapeBBClientMixin`** — rotates the custom loop-selection outline into the wheel plane.
...
Gradle resolves Create 6.0.10 from its official Modrinth artifact and resolves Flywheel, Ponder, and Registrate from their published Maven repositories. The development run configurations include Create at runtime, while the built mod remains a thin jar and does not bundle those dependencies.
