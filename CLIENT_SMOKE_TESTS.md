...
- **Forced recovery:** for an unrouted package with a valid fallback, confirm it takes a non-incoming fallback. If every other exit is blocked, confirm it only returns to the incoming side after a full loop.
